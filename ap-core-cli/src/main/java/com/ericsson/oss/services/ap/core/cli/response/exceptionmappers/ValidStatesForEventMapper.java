...
public class ValidStatesForEventMapper {
    ...
    public String getValidStates(final String commandName) {
        final List<StateTransitionEvent> stateTransitionEvents = COMMAND_TRANSITION_EVENTS.get(commandName.toLowerCase(Locale.ENGLISH));

        if (stateTransitionEvents == null) {
            return "";
        }

        return buildValidStatesResult(stateTransitionEvents);
    }

    private static String buildValidStatesResult(final List<StateTransitionEvent> stateTransitionEvents) {
