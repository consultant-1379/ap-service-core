...
public class QueryAllProjectsRestTest extends ViewTest {

    @Inject
    private Stubs stubs;

    public QueryAllProjectsRestTest() {
        super("projectName", "creator", "creationDate", "description", "parent", "children");
    }

    @When("a service exception occurs on the server")
    public void service_exception_occurs_on_the_server(){
        final AutoProvisioningService autoProvisioningService = stubs.injectIntoSystem(AutoProvisioningService.class);
        doThrow(ApServiceException.class).when(autoProvisioningService).viewAllProjects();
    }
}
