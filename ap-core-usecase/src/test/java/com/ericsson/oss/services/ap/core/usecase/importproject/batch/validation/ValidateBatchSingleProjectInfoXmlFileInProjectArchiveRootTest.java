...
/**
 * Unit tests for {@link ValidateBatchSingleProjectInfoXmlFileInProjectArchiveRoot}.
 */
@RunWith(MockitoJUnitRunner.class)
public class ValidateBatchSingleProjectInfoXmlFileInProjectArchiveRootTest {
    ...
    private static final String EXPECTED_VALIDATION_MSG_FILE_NOT_FOUND = "No projectInfo.xml file found in project file";

    @InjectMocks
    private ValidateBatchSingleProjectInfoXmlFileInProjectArchiveRoot validateBatchSingleProjectInfoXmlFileInProjectArchiveRoot;

    @Test
    public void when_projectInfo_is_present_in_archive_then_ap_batch_validation_returns_true() {
        final ZipContentGenerator zcg = new ZipContentGenerator();
        zcg.createFileInZip(PROJECT_INFO_XML, TEST_CONTENT);
        ...
        final boolean result = validateBatchSingleProjectInfoXmlFileInProjectArchiveRoot.execute(context);

        assertTrue(result);
    }

    @Test
    public void when_projectInfo_is_not_present_in_archive_then_ap_batch_validation_returns_false() {
        final ZipContentGenerator zcg = new ZipContentGenerator();
        zcg.createFileInZip(INVALID_FILE_TXT, TEST_CONTENT);
        final ValidationContext context = new ValidationContext(PROJECT_BATCH_VALIDATION_GROUP, zcg.getZipData(VALID_PROJECT_ZIP_FILE_NAME));

        validateBatchSingleProjectInfoXmlFileInProjectArchiveRoot.execute(context);
