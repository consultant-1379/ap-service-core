...
class ValidateExpansionNodeTypeSupportedSpec extends CdiSpecification {
    ...
    @ObjectUnderTest
    private ValidateExpansionNodeTypeSupported validateExpansionNodeTypeSupported;
    ...
    private static final String INVALID_EXPANSION_NODE_TYPE_2 = "ERBS";
    private static final String SUPPORTED_EXPANSION_NODE_TYPE = "RadioNode";

    private static final List<String> SUPPORTED_NODE_TYPES = Arrays.asList(INVALID_EXPANSION_NODE_TYPE_2, SUPPORTED_EXPANSION_NODE_TYPE);
    ...
    def setup() {
        ...
        nodeTypeMapper.getInternalEjbQualifier(SUPPORTED_EXPANSION_NODE_TYPE) >> "ecim"
        nodeTypeMapper.getInternalEjbQualifier(INVALID_EXPANSION_NODE_TYPE_2) >> "erbs"
    ...
    def "Verify validation passed when corresponding node type is supported" () {
        ...
        addNetworkElementMo(SUPPORTED_EXPANSION_NODE_TYPE)
        ...
        autoProvisioningWorkflowService.getExpansionOrderWorkflowName() >> "expansion_order_ecim_1"

        when: "node type is supported"
        boolean isNodeTypeSupp = validateExpansionNodeTypeSupported.execute(validationContext)
    ...
    def "Verify validation failed when corresponding node type is not supported" () {
        given: "Expansion nodeInfo xml"
        addNetworkElementMo(INVALID_EXPANSION_NODE_TYPE_2)
        nodeInfoArtifact = new ArchiveArtifact("NodeInfo", getNodeInfo("Node1"))
        archive.getArtifactOfNameInDir(NODE_NAME, NODE_INFO) >> nodeInfoArtifact
        autoProvisioningWorkflowServiceUnsupportedNode.getExpansionOrderWorkflowName() >> null
