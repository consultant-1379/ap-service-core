...
import com.ericsson.oss.services.ap.core.usecase.validation.replace.ValidateHardwareReplaceNodeTypeSupported;
import com.ericsson.oss.services.ap.core.usecase.workflow.ApWorkflowServiceResolver

class ValidateHardwareReplaceNodeTypeSupportedSpec extends CdiSpecification {
    ...
    @ObjectUnderTest
    private ValidateHardwareReplaceNodeTypeSupported validateHardwareReplaceNodeTypeSupported;
    ...
    private static final String INVALID_HARDWARE_REPLACE_NODE_TYPE = "ERBS";
    private static final String SUPPORTED_HARDWARE_REPLACE_NODE_TYPE = "RadioNode";

    private static final List<String> SUPPORTED_NODE_TYPES = Arrays.asList(INVALID_HARDWARE_REPLACE_NODE_TYPE, SUPPORTED_HARDWARE_REPLACE_NODE_TYPE);
    ...
    def setup() {
        ...
        validationContext = new ValidationContext("hardwarereplace", projectDataContentTarget);

        modelReader.getSupportedNodeTypes() >> SUPPORTED_NODE_TYPES;

        nodeTypeMapper.getInternalEjbQualifier(SUPPORTED_HARDWARE_REPLACE_NODE_TYPE) >> "ecim"
        nodeTypeMapper.getInternalEjbQualifier(INVALID_HARDWARE_REPLACE_NODE_TYPE) >> "erbs"
    ...
    def "Verify validation passed when corresponding node type is supported" () {
        given: "Hardware Replace nodeInfo xml"
        MoCreatorSpec.createNetworkElementMo(NODE_NAME, SUPPORTED_HARDWARE_REPLACE_NODE_TYPE, null)
        ...
        autoProvisioningWorkflowService.getHardwareReplaceWorkflowName() >> "hardware_replace_order_ecim_1"

        when: "node type is supported"
        boolean isNodeTypeSupp = validateHardwareReplaceNodeTypeSupported.execute(validationContext)
    ...
    def "Verify validation failed when corresponding node type is not supported" () {
        given: "Hardware Replace nodeInfo xml"
        MoCreatorSpec.createNetworkElementMo(NODE_NAME, INVALID_HARDWARE_REPLACE_NODE_TYPE, null)
        nodeInfoArtifact = new ArchiveArtifact("NodeInfo", getNodeInfo("Node1"))
        archive.getArtifactOfNameInDir(NODE_NAME, NODE_INFO) >> nodeInfoArtifact
        autoProvisioningWorkflowServiceUnsupportedNode.getHardwareReplaceWorkflowName() >> null
