...
/**
 * Unit tests for {@link MigrationFailedTaskTest}.
 */
@RunWith(MockitoJUnitRunner.class)
public class MigrationFailedTaskTest extends AbstractFailedTaskTest {

    @InjectMocks
    private MigrationFailedTask migrationFailedTask;
    ...
    @Test
    public void whenMigrationFailedThenStateChangesToMigrationFailed() {
        migrationFailedTask.executeTask(execution);
        verify(dpsOperations, times(1)).updateMo(NODE_FDN, "activeWorkflowInstanceId", null);
        verify(commandRecorder, times(1)).migrationFailed(workflowVariables);
        verify(stateTransitionManager, times(1)).validateAndSetNextState(NODE_FDN, StateTransitionEvent.MIGRATION_FAILED);
    }
}
