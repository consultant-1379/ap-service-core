...
public abstract class AbstractEoiCancelEnrollmentTask extends AbstractServiceTask {
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        try {
            eoiCancelEnrollment(apNodeFdn,baseUrl,cookieValue);
        } catch (final Exception e) {
            logger.error("EoiCancelEnrollmentTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());
            workflowVariables.setEoiRollbackError(true);
        }
    }
    protected abstract void eoiCancelEnrollment(final String apNodeFdn,final String baseUrl ,final String cookieValue) ;
