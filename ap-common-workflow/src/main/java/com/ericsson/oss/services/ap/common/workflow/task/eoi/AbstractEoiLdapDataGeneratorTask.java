...
public abstract class AbstractEoiLdapDataGeneratorTask extends AbstractServiceTask{
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        try {
            eoiLdapDataGenerator(apNodeFdn, baseUrl, cookieValue);
    } catch (final Exception e) {
            logger.error("EoiLdapDataGeneratorTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());

        throwBpmnError(BpmnErrorKey.EOI_INTEGRATION_WORKFLOW_ERROR_KEY, e.getMessage(), taskExecution);
    }
}

    protected abstract void eoiLdapDataGenerator(final String nodeFdn, final String baseUrl, final String cookieValue);
