...
public abstract class AbstractEoiGenerateSecurityCredentialTask extends AbstractServiceTask {
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        final String baseUrl = workflowVariables.getBaseUrl();
        final String cookieValue = workflowVariables.getSessionId();
        try {
            eoiGenerateSecurityCredentials(apNodeFdn,baseUrl,cookieValue);
        } catch (final Exception e) {
            logger.error("EoiGenerateSecurityCredentialTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());

            throwBpmnError(BpmnErrorKey.EOI_INTEGRATION_WORKFLOW_ERROR_KEY, e.getMessage(), taskExecution);
        }
    }

    protected abstract void eoiGenerateSecurityCredentials(final String nodeFdn,final String baseUrl,final String cookieValue);
