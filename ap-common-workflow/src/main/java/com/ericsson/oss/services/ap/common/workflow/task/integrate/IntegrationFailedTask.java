...
/**
 * Task executed after failure of an ongoing integration, which sets the AP <code>NodeStatus</code> <i>state</i> to <b>INTEGRATION_FAILED</b>.
 */
public class IntegrationFailedTask extends AbstractServiceTask {
    ...
    public void executeTask(final TaskExecution execution) {
        ...
        setNodeStateToIntegrationFailed(apNodeFdn);
        commandRecorder.integrationFailed(workflowVariables);
        dpsOperations.updateMo(apNodeFdn, NodeAttribute.ACTIVE_WORKFLOW_INSTANCE_ID.toString(), null);
    }

    private static void setNodeStateToIntegrationFailed(final String apNodeFdn) {
        final StateTransitionManagerLocal stateTransitionManager = new ServiceFinderBean().find(StateTransitionManagerLocal.class);
        stateTransitionManager.validateAndSetNextState(apNodeFdn, StateTransitionEvent.INTEGRATION_FAILED);
    }
}
