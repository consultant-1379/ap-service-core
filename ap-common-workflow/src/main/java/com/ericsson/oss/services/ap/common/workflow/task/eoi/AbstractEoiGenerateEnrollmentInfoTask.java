...
public abstract class AbstractEoiGenerateEnrollmentInfoTask extends AbstractServiceTask {
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        try {
            eoiGenerateEnrollmentInfoTask(apNodeFdn, baseUrl, cookieValue);
        } catch (final Exception e) {
            logger.error("EoiGenerateEnrollmentInfoTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());

            throwBpmnError(BpmnErrorKey.EOI_INTEGRATION_WORKFLOW_ERROR_KEY, e.getMessage(), taskExecution);
        }
    }

    protected abstract void eoiGenerateEnrollmentInfoTask(final String nodeFdn, final String baseUrl, final String cookieValue);
}
