...
public abstract class AbstractEoiSnmpConfigurationTask extends AbstractServiceTask {
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        try {
            eoiSnmpConfigurationTask(apNodeFdn, baseUrl, cookieValue);
     } catch (final Exception e) {
            logger.error("EoiSnmpConfigurationTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());

            throwBpmnError(BpmnErrorKey.EOI_INTEGRATION_WORKFLOW_ERROR_KEY, e.getMessage(), taskExecution);
    }
}

    protected abstract void eoiSnmpConfigurationTask(final String nodeFdn, final String baseUrl, final String cookieValue);
}
