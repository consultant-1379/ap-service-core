...
public abstract class AbstractEoiGenerateArtifactTask extends AbstractServiceTask {
    ...
    public void executeTask(TaskExecution taskExecution) {
        ...
        try {
                eoiGenerateArtifact(apNodeFdn);
            } catch (final Exception e) {
            logger.error("EoiGenerateArtifactTask is failed for the node fdn : {} with reason : {}", apNodeFdn, e.getMessage());

                throwBpmnError(BpmnErrorKey.EOI_INTEGRATION_WORKFLOW_ERROR_KEY, e.getMessage(), taskExecution);
            }
        }

        protected abstract void eoiGenerateArtifact(final String apNodeFdn);
}
